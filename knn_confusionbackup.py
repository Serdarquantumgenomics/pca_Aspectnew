...
def nn(K):
    ...
    dataobject_x=nib.load('./ATTbasedFlowTerritories_HCP.nii.gz')
    ...
    conf_mat=confusion_matrix(test_y.argmax(axis=1), preds_kNN.argmax(axis=1))     
    print(conf_mat) 

    np.save('./output/conf_mat'+str(K)+'.npy',conf_mat)
