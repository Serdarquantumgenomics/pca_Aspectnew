...
def s_gen(K):
 ...
 #   K=100
    ...
    data_projected=thepca.transform((s_m))
#    data_backprojected=thepca.inverse_transform(data_projected)
    ...
    eigen=thefit.components_[0:5]
    eigen=eigen.reshape([-1,5])
    ...
    s[index3[0],:5]=eigen
    print('before save')    
    data_matrix=np.transpose(s[:,:5]).reshape([dim_0,dim_1,dim_2,5])
    #data_matrix=(s).reshape([dim_0,dim_1,dim_2,4036])
    new=nib.Nifti1Image(data_matrix,affine_x)
    nib.save(new,'./output/eigen5.nii.gz')
